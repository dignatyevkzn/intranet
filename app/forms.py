from app.models import db, User, Direction, Who_number, Customer, Truck, Quantity, Truck_opt, Ttn, Customer_base
...
from wtforms.ext.sqlalchemy.fields import QuerySelectField, SelectFieldBase
...
class NewRequestForm(FlaskForm):
    ...
    cargo_type = QuerySelectField ('Тип перевозки', query_factory=choice_truck, get_label='name')
    ...
    type_of_loading = SelectField\
        ('Способ погрузки: ',
         choices= [('зад', 'зад'),
         ('бок', 'бок')], validators=[DataRequired()])
    type_of_truck = StringField('Тип транспорта', validators=[DataRequired()])
    weigth_cargo = SelectField('Вес груза:',
                               choices=[('до 500 кг', ' 500 кг'),
                                        ('до 1.5тн', 'до 1.5кг'),
                                        ('до 3тн', 'до 3тн')
                                        ],
...
# def choice_request():
#     from app import Request, current_user
...
class CustomerForm(FlaskForm):
    ...
    payment_terms = QuerySelectField('Копия или Оригинал?', query_factory=choice_ttn, get_label='name')
    customer_base = QuerySelectField('Какая база клиентов используется?', query_factory=choice_base, get_label='name')
